...
export function ThreadEditor({ availableTags }: ThreadEditorProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/threads/${data.thread.threadId}`);
    } finally {
      setLoading(false);
    }
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Décris ton problème ou ton sujet en détail…"
          required
          minLength={10}
          rows={10}
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {loading ? "Publication…" : "Publier le thread"}
        </button>
      </div>
    </form>
  );
